    ...
    <el-row>
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          <span v-if="!isEdit">Create Grpc Service</span>
          <span v-if="isEdit">Update Grpc Service</span>
        </div>
  ...
  methods: {
    fetchFormData(id) {
      showService(id).then((response) => {
        ...
        this.form.header_transform = response.data.grpc_rule.header_transform.replace(
          /,/g,
          '\n'
        )
    ...
    handleSubmit() {
      ...
      console.log(body)
      if (this.isEdit) {
        updateGrpcService(this.service_id, body)
          .then((response) => {
            this.isSubmit = false
